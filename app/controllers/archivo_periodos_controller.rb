class ArchivoPeriodosController < ApplicationController
	def index 		
		x  = Docente.find_by(email: current_usuario.email)
            if params[:periodo] != nil                 
                ...
                if @archivo != nil
                    @archivos = ArchivoPeriodo.where(periodo_curs_materia_id: @archivo.id)
                else
                   @archivos = []
                end
            else
	...
	def new	
		@archivo = ArchivoPeriodo.new
		@periodo = PeriodoCursMateria.find(params[:periodo_curs_materia_id])
	end
	def create
		@archivo = ArchivoPeriodo.create(archivo_params)		
		@periodo = PeriodoCursMateria.find(archivo_params[:periodo_curs_materia_id])
		if @archivo.save
			redirect_to periodo_curs_materia_archivo_periodos_path(@periodo)
		else
			render :new
		end		
	end
	def show
		@archivo = ArchivoPeriodo.find(params[:id])
		@periodo = PeriodoCursMateria.find(params[:periodo_curs_materia_id])
	end
	def destroy		
		@archivo = ArchivoPeriodo.find(params[:id])
        ...
        @archivo.destroy
        redirect_to periodo_curs_materia_archivo_periodos_path(@curso_materia)
	end
	def edit
		@archivo = ArchivoPeriodo.find(params[:id])
	...
	private
	def archivo_params
		params.require(:archivo_periodo).permit(:archivo, :descripcion, :periodo_curs_materia_id)
	end
end
